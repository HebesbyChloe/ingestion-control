...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use Gateway endpoint: /typesense/metrics
    const gatewayUrl = `${API_GATEWAY_URL}/typesense/metrics`;
    ...
    if (!response.ok) {
      ...
      console.error('Gateway metrics error:', {
        status: response.status,
        error: errorText,
      });
      
      return NextResponse.json(
        { error: 'Failed to fetch metrics from gateway', details: errorText },
        { status: response.status }
      );
    }
