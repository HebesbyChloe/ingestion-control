...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use Gateway endpoint: /typesense/stats
    const gatewayUrl = `${API_GATEWAY_URL}/typesense/stats`;
    ...
    if (!response.ok) {
      ...
      console.error('Gateway stats error:', {
        status: response.status,
        error: errorText,
      });
      
      return NextResponse.json(
        { error: 'Failed to fetch stats from gateway', details: errorText },
        { status: response.status }
      );
    }
